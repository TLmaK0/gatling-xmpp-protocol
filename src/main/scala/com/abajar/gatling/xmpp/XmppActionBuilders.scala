...
class XmppConnectActionBuilder(requestName: Expression[String], nodeName: Expression[String]) extends ActionBuilder {
  override def build(next: ActorRef, protocols: Protocols): ActorRef = actor(actorName("xmppConnect"))(new XmppConnectAction(requestName, next,
    protocols.getProtocol[XmppBoshProtocol].getOrElse(throw new UnsupportedOperationException("XmppBosh Protocol wasn't registered")), nodeName))
}
